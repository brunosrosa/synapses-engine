...
class DocumentChangeHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        logger.info(f"📝 Documento modificado: {file_path}")
    ...
    def on_created(self, event):
        ...
        logger.info(f"📄 Novo documento: {file_path}")
        self.sync_manager.mark_for_sync(file_path)
    ...
    def on_deleted(self, event):
        ...
        logger.info(f"🗑️ Documento removido: {file_path}")
...
class RAGSyncManager:
    ...
    def _load_sync_state(self):
        ...
        if self.sync_state_file.exists():
            try:
                ...
                logger.info(f"📂 Estado de sincronização carregado: {len(self.file_hashes)} arquivos")
                
            except Exception as e:
                logger.error(f"❌ Erro ao carregar estado de sincronização: {e}")
    ...
    def _save_sync_state(self):
        ...
        try:
            ...
            logger.error(f"❌ Erro ao salvar estado de sincronização: {e}")
    ...
    def scan_for_changes(self) -> Dict[str, List[Path]]:
        ...
        logger.info(f"🔍 Escaneando mudanças em: {self.source_dir}")
        ...
        logger.info(f"📊 Mudanças encontradas: {total_changes} ({len(changes['new'])} novos, {len(changes['modified'])} modificados, {len(changes['deleted'])} deletados)")
    ...
    async def sync_changes(self, force_full_reindex: bool = False) -> Dict[str, any]:
        ...
        logger.info("🔄 Iniciando sincronização RAG...")
        ...
        try:
            ...
            if force_full_reindex:
                logger.info("🔄 Executando re-indexação completa...")
                ...
                if changes_count > 0:
                    logger.info(f"📝 Processando {changes_count} mudanças...")
    ...
    def start_monitoring(self):
        ...
        logger.info(f"👁️ Iniciando monitoramento de: {self.source_dir}")
    ...
    async def run_auto_sync_loop(self):
        ...
        logger.info(f"🔄 Iniciando loop de sincronização automática (intervalo: {self.auto_sync_interval}s)")
        ...
        while True:
            try:
                ...
                if self.pending_files or self.removed_files:
                    logger.info("🔄 Executando sincronização automática...")
                    result = await self.sync_changes()
                    
                    if result['success']:
                        logger.info(f"✅ Sincronização automática concluída: {result['changes_processed']} mudanças")
                    else:
                        logger.error(f"❌ Falha na sincronização automática: {result['errors']}")
...
async def main():
    ...
    try:
        if args.mode == 'status':
            ...
            print("🔄 Executando re-indexação completa...")
            result = await sync_manager.sync_changes(force_full_reindex=True)
            
            if result['success']:
                print(f"✅ Re-indexação concluída em {result['duration']:.2f}s")
            else:
                print(f"❌ Falha na re-indexação: {result['errors']}")
            ...
            if result['success']:
                print(f"✅ Sincronização concluída: {result['changes_processed']} mudanças em {result['duration']:.2f}s")
            else:
                print(f"❌ Falha na sincronização: {result['errors']}")
            ...
            if args.auto_sync:
                print("🔄 Sincronização automática ativada")
