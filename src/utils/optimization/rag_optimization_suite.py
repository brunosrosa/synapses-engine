...
class RAGOptimizationSuite:
    ...
    def run_full_optimization(self) -> Dict[str, OptimizationResult]:
        ...
        print("[START] SUÍTE DE OTIMIZAÇÃO RAG - EXECUÇÃO COMPLETA")
        print("=" * 60)
        
        # 1. Setup de otimizações
        print("\n1[EMOJI]⃣ Configurando otimizações...")
        results['setup'] = self.setup_optimizations()
        
        # 2. Conversão de embeddings
        print("\n2[EMOJI]⃣ Convertendo embeddings...")
        results['conversion'] = self.converter.convert_numpy_to_pytorch()
        
        # 3. Benchmark de performance
        print("\n3[EMOJI]⃣ Executando benchmark...")
    ...
    def _print_optimization_summary(self, results: Dict[str, OptimizationResult]):
        ...
        print("\n" + "=" * 60)
        print("[EMOJI] RESUMO DA OTIMIZAÇÃO")
        print("=" * 60)
        
        successful = sum(1 for r in results.values() if r.success)
        total = len(results)
        
        print(f"[OK] Operações bem-sucedidas: {successful}/{total}")
        print(f"[ERROR] Operações com falha: {total - successful}/{total}")
        print(f"⏱[EMOJI]  Tempo total: {sum(r.execution_time for r in results.values()):.3f}s")
