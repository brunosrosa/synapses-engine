...
try:
    ...
    from rag_infra.src.core.core_logic.rag_metrics_monitor import metrics_collector, start_monitoring, stop_monitoring
    from rag_infra.src.core.core_logic.rag_metrics_integration import apply_metrics_tracking
...
def print_results_summary(results: LoadTestResults):
    ...
    if results.avg_response_time < 500:
        print("   [OK] Tempo de resposta: EXCELENTE (<500ms)")
    elif results.avg_response_time < 1000:
        print("   [OK] Tempo de resposta: BOM (<1000ms)")
    elif results.avg_response_time < 2000:
        print("   [WARNING]  Tempo de resposta: ACEITÁVEL (<2000ms)")
    else:
        print("   [ERROR] Tempo de resposta: PRECISA MELHORAR (>2000ms)")
    
    if results.error_rate < 0.01:
        print("   [OK] Taxa de erro: EXCELENTE (<1%)")
    elif results.error_rate < 0.05:
        print("   [OK] Taxa de erro: BOA (<5%)")
    else:
        print("   [ERROR] Taxa de erro: ALTA (>5%)")
    
    if results.cache_hit_rate > 0.7:
        print("   [OK] Cache hit rate: EXCELENTE (>70%)")
    elif results.cache_hit_rate > 0.5:
        print("   [OK] Cache hit rate: BOA (>50%)")
    else:
        print("   [WARNING]  Cache hit rate: PODE MELHORAR (<50%)")
