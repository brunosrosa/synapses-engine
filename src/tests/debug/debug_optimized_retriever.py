...
def test_optimized_retriever_direct():
    ...
    try:
        ...
        logger.info(f"Inicialização: {'[OK] SUCESSO' if init_success else '[ERROR] FALHA'}")
    ...
    try:
        ...
        if results:
            ...
            if hasattr(retriever.pytorch_retriever, 'base_retriever'):
                ...
                if base_results:
                    logger.info("[OK] Base retriever funciona - problema na delegação")
                else:
                    logger.error("[ERROR] Base retriever também não funciona")
...
def main():
    ...
    logger.info(f"RAGRetriever com otimizações: {'[OK] SUCESSO' if rag_success else '[ERROR] FALHA'}")
    
    if direct_success and not rag_success:
        logger.warning("[WARNING] OptimizedPyTorchRetriever funciona diretamente, mas não através do RAGRetriever")
        logger.warning("[WARNING] Problema na integração ou delegação")
