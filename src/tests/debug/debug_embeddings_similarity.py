...
def debug_embeddings_and_similarity():
    ...
    try:
        ...
        print("[OK] RAGRetriever inicializado com sucesso")
    ...
    if retriever.documents:
        ...
        print("[ERROR] Nenhum documento encontrado")
        return
    ...
    if not embedding_model.load_model():
        print("[ERROR] Falha ao carregar modelo de embedding")
        return
    ...
    for query in test_queries:
        ...
        try:
            ...
            if query_embedding is None:
                print(f"[ERROR] Falha ao gerar embedding para '{query}'")
                continue
            print(f"[OK] Embedding gerado: shape={query_embedding.shape}, norm={np.linalg.norm(query_embedding):.4f}")
            ...
            if hasattr(retriever, 'pytorch_retriever') and retriever.pytorch_retriever:
                ...
                if hasattr(base_retriever, 'embeddings_tensor') and base_retriever.embeddings_tensor is not None:
                    ...
                    print(f"[OK] Embeddings dos documentos: shape={doc_embeddings_tensor.shape}, device={doc_embeddings_tensor.device}")
                    ...
                    if len(doc_embeddings) > 0:
                        ...
                        print("[ERROR] Nenhum embedding de documento encontrado")
                else:
                    print("[ERROR] Atributo 'embeddings_tensor' não encontrado no retriever")
            ...
            try:
                ...
                if results:
                    ...
                    print("[ERROR] Busca real não retornou resultados")
            except Exception as e:
                print(f"[ERROR] Erro na busca real: {e}")
                
        except Exception as e:
            print(f"[ERROR] Erro ao processar query '{query}': {e}")
    ...
    if hasattr(retriever, 'pytorch_retriever'):
        ...
        for attr in attrs_to_check:
            if hasattr(base_ret, attr):
                ...
                print(f"  {attr}: [ERROR] Não encontrado")
