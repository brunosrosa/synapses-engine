...
def test_retriever_initialization():
    ...
    try:
        # Adicionar path
        core_logic_path = Path(__file__).parent / 'rag_infra' / "src/core/core_logic"
        ...
        logger.info("[OK] rag_retriever importado com sucesso")
        
        # Obter retriever
        logger.info("Obtendo retriever...")
        retriever = get_retriever()
        logger.info(f"[OK] Retriever obtido: {type(retriever)}")
        ...
        if result:
            logger.info("[OK] Inicialização bem-sucedida")
            ...
            if load_result:
                logger.info("[OK] Índice carregado com sucesso")
