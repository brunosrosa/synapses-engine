...
def test_rag_system():
    ...
    try:
        ...
        logger.info("📦 Importando RAGRetriever...")
        from core_logic.rag_retriever import RAGRetriever
        logger.info("✅ Import bem-sucedido")
        
        # Criar instância
        logger.info("🔧 Criando instância do RAG retriever...")
        ...
        if not init_result:
            logger.error("❌ Falha na inicialização do sistema RAG")
            return False
        
        # Verificar estado
        logger.info(f"📊 Estado do sistema RAG:")
        ...
        if retriever.is_loaded:
            logger.info("🔍 Testando busca no sistema RAG...")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("🎉 Sistema RAG com PyTorch GPU está funcionando corretamente!")
    else:
        print("\n=== Resultado Final: FALHA ===")
        print("❌ Problemas detectados no sistema RAG")