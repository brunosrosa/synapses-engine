...
def test_mcp_server_import():
    ...
    try:
        ...
        if has_list_tools and has_call_tool:
            logger.info("✅ MCP Server funcionando corretamente")
            return True
        else:
            logger.error("❌ MCP Server sem funcionalidades necessárias")
            return False
            
    except Exception as e:
        logger.error(f"❌ Erro no teste MCP Server: {str(e)}")
        import traceback
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("🎉 MCP Server está funcionando corretamente!")
    else:
        print("\n=== Resultado Final: FALHA ===")
        print("❌ Problemas detectados no MCP Server")
        sys.exit(1)