...
class IndexConsistencyChecker(UtilityInterface):
    ...
    def execute(self) -> UtilityResult:
        ...
        try:
            print("🔍 Verificando consistência do índice...")
            print("=" * 50)
            ...
            print(f"FAISS Index: {'✅ Consistente' if faiss_consistent else '❌ Inconsistente'}")
            print(f"PyTorch Index: {'✅ Consistente' if pytorch_consistent else '❌ Inconsistente'}")
...
class RAGUtilitiesRunner:
    ...
    def run_all_utilities(self) -> Dict[str, UtilityResult]:
        ...
        print("🛠️  UTILITÁRIOS RAG - EXECUÇÃO COMPLETA")
        print("=" * 60)
        ...
        for utility_name in self.utilities.keys():
            try:
                ...
                print(f"❌ {utility_name}: Erro inesperado: {e}")
    ...
    def _print_summary(self):
        ...
        print(f"⏱️  Tempo total: {sum(r.execution_time for r in self.results.values()):.3f}s")
        
        if total - successful > 0:
            print("\n🚨 UTILITÁRIOS COM FALHAS:")
            for name, result in self.results.items():
                if not result.success:
                    print(f"   • {name}: {result.message}")
        
        print("\n🏁 UTILITÁRIOS CONCLUÍDOS")
